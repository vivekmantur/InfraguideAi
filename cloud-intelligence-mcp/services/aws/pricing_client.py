...
class AwsPricingClient:
    ...
    def _fargate_rates(self, region: str) -> FargateRates:
        ...
        usage_prefix = AWS_REGION_USAGE_PREFIXES[region]
        vcpu_products = self._get_products(
            "AmazonECS",
            [
                {"Type": "TERM_MATCH", "Field": "location", "Value": location},
                {
                    "Type": "TERM_MATCH",
                    "Field": "usagetype",
                    "Value": f"{usage_prefix}-Fargate-vCPU-Hours:perCPU",
                },
            ],
        )
        memory_products = self._get_products(
            "AmazonECS",
            [
                {"Type": "TERM_MATCH", "Field": "location", "Value": location},
                {
                    "Type": "TERM_MATCH",
                    "Field": "usagetype",
                    "Value": f"{usage_prefix}-Fargate-GB-Hours",
                },
            ],
        )
